...
def make_digital_out(pin):
    ...
    return Pin(resolve_pin(pin), Pin.OUT)
...
def make_digital_in(pin, pull_up=False):
    ...
    Prints a warning if the GPIO is a boot-strapping pin, which can
    cause boot failures or force the chip into download mode.
    """
    from machine import Pin
